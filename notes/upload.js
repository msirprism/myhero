...
const uploadLink = createUploadLink({
  uri: 'http://localhost:4467', // Apollo Server is served from port 4000
  headers: {
    "keep-alive": "true"
  }
})
...
const UPLOAD_FILE = gql`
  mutation UploadUserPhoto($id: Int!, $file: Upload!) {
    uploadUserPhoto(id: $id, photo: $file) {
      id
      name
      photo
    }
                    ...
                    {(uploadUserPhoto, { data, loading }) => {
                        ...
                        return (<form onSubmit={() => {console.log("Submitted")}} encType={'multipart/form-data'}>
                                    <input name={'document'} type={'file'} onChange={({target: { files }}) => {
                                        const file = files[0]
                                        file && uploadUserPhoto({ variables: { id: 1, file: file } })
